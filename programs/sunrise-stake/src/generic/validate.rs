    ...
    U: From<SplAccounts<'info>>,
{
    ...
    assert!(manager.spl_pools.contains(pool.key));
    let pool_account = Account::<'info, SplWrapper>::try_from(&pool)?;
    ...
    let sunrise_pool_mint_token_account = Account::<'info, TokenAccount>::try_from(&sunrise_pool_mint_token_account)?;
    let token_account_authority = next_account_info(accounts_iter)?; 
    assert_eq!(token_account_authority.key(), manager.generic_token_account_auth);
    assert_eq!(sunrise_pool_mint_token_account.owner, token_account_authority.key());
    ...
    let spl_accounts: Box<U> = Box::new(
        SplAccounts {
            ...
            sunrise_pool_mint_token_account: sunrise_pool_mint_token_account,
    ...
    U: From<MarinadeAccounts<'info>>,
{
    ...
    assert_eq!(marinade_state.key(), state.marinade_state);
    let marinade_state_account = Box::new(Account::<'info, MarinadeState>::try_from(&marinade_state)?);
    let msol_mint = next_account_info(accounts_iter)?;
    assert_eq!(marinade_state_account.msol_mint, msol_mint.key());
    ...
    let liq_pool_msol_leg_account = Box::new(Account::<'info, TokenAccount>::try_from(&liq_pool_msol_leg_account)?);
    ...
    let sunrise_msol_token_account = Box::new(Account::<'info, TokenAccount>::try_from(&sunrise_msol_token_account)?);
    let sunrise_liq_pool_token_account = next_account_info(accounts_iter)?;
    let sunrise_liq_pool_token_account = Box::new(Account::<'info, TokenAccount>::try_from(&sunrise_liq_pool_token_account)?);
    let sunrise_msol_account_authority = next_account_info(accounts_iter)?;
    msg!("c");
    assert_eq!(sunrise_msol_token_account.owner, sunrise_msol_account_authority.key());
    assert_eq!(sunrise_msol_token_account.mint, msol_mint.key());
    assert_eq!(sunrise_liq_pool_token_account.owner, sunrise_msol_account_authority.key());
    assert_eq!(sunrise_liq_pool_token_account.mint, marinade_state_account.liq_pool.lp_mint);

    let (pda, bump) = Pubkey::find_program_address(&[state.key().as_ref(), seeds::MSOL_ACCOUNT], &crate::ID);
    assert_eq!(pda, sunrise_msol_account_authority.key());
    assert_eq!(bump, state.msol_authority_bump);
