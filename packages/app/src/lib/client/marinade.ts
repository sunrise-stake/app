...
import {
    ...
    findOrderUnstakeTicketManagementAccount, findPrePoolAccount,
    SunriseStakeConfig
} from "./util";
...
const getOrderUnstakeTicketManagementAccount = async (
    ...
    epoch: number
): Promise<{ address: PublicKey, bump: number, account: ManagementAccount | null}> => {
    const [address, bump] = findOrderUnstakeTicketManagementAccount(config, BigInt(epoch));
...
export const triggerRebalance = async (
    ...
    const epochInfo = await program.provider.connection.getEpochInfo();
    const managementAccount = await getOrderUnstakeTicketManagementAccount(config, program, epochInfo.epoch);
    const previousManagementAccount = await getOrderUnstakeTicketManagementAccount(config, program, epochInfo.epoch- 1);
    const [prePoolAccount, prePoolAccountBump] = findPrePoolAccount(config)
    const maintenanceInstructions = [];

    // TODO Add instruction to close an arbitrary ticket, in case an epoch gets missed

    const previousEpochTickets = previousManagementAccount.account ? await findAllTickets(
    ...
    const accounts: Accounts = {
        ...
        previousOrderUnstakeTicketManagementAccount: previousManagementAccount.account ? previousManagementAccount.address : null,
        systemProgram: SystemProgram.programId,
        tokenProgram: TOKEN_PROGRAM_ID,
