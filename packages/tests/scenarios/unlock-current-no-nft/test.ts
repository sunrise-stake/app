...
describe("unlock-current-no-nft", () => {
  ...
  it("unlocks directly when lock account is current and no NFT exists", async () => {
    log("Attempting to unlock gSOL (no NFT, already current)...");

    // When the lock account is already up to date and there's no NFT,
    // unlockGSol should skip the update and proceed directly to unlock
