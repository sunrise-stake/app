...
describe('sunrise-stake', () => {
  ...
  let stakerMsolTokenAccountAuthority: PublicKey
  ...
  before('Initialise state', async () => {
    ...
    if (!client.config) throw new Error('Client config not found')

    stakerMsolTokenAccountAuthority = findMSolTokenAccountAuthority(client.config, client.staker)[0]
  })

  before('Initialise staker', async () => await client.createGSolTokenAccount())
  before('log stuff', async () => await client.details().then(console.log))
